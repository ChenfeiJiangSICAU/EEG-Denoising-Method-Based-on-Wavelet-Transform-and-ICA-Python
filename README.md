...
`eeg_denoise.py` 用于对单个 EDF 头皮 EEG 进行离线强去噪，并输出压缩 NPZ。
...
- 当前处理只保留 `brain` 较激进，可能误删真实脑活动，必须检查波形、频谱和 ICA 标签，具体详见`8.` ，可根据研究方向自行筛选保留标签信号。
...
`eeg_denoise.py` performs offline strong denoising of one scalp-EEG EDF and writes a compressed NPZ.
