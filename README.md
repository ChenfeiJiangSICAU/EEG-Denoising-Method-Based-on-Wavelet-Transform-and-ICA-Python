...
`eeg_denoise_strong.py` 用于对单个 EDF 头皮 EEG 进行离线强去噪，并输出压缩 NPZ。
强去噪指：ICLabel 分类后只保留 `brain` 成分，删除全部非 `brain` 成分，不设置信度阈值。

## 2. 使用场景与效果

- 适用于标准单极头皮 EEG、常规 10-20/10-10 通道布局和 0.5–45 Hz 分析。
- 可降低基线漂移、50 Hz 工频、高频随机噪声及 ICLabel 判定的非脑成分。
- 适合离线预处理、后续统计、建模及去噪前后质量检查。
- 不适合双极导联、颅内 EEG、实时处理或需要保留 45 Hz 以上活动的研究。
- 当前处理只保留 `brain` 较激进，可能误删真实脑活动，必须检查波形、频谱和 ICA 标签，具体详见8. ，可根据研究方向自行筛选保留标签信号。

## 3. 主要流程

1. 使用 MNE 读取并预加载一个 EDF；目录输入只取排序后的第一个 EDF。
2. 清理通道名，匹配 `standard_1020`，删除非 EEG 和已标记坏道。
3. 执行 0.5 Hz 高通；50 Hz 低于 Nyquist 时执行陷波。
4. 各通道按 8 秒、50% 重叠进行 `sym8` 六层小波去噪。
5. 转为全脑平均参考，并在目标 1–100 Hz 分支拟合 extended Infomax ICA。
6. 使用 ICLabel 分类，只保留 `brain`，删除所有非 `brain` 成分。
...
| ICLabel 默认 | 仅保留 `brain` | 强去噪；无置信度阈值，误删风险较高 |
| 最终滤波 | 45 Hz 低通 | 抑制高频残余 |
| 绘图 | 前 40 秒 | 黑色去噪前、红色去噪后 |

## 8. 按研究方向选择 ICLabel 标签

去噪强度可通过 ICA 的 `exclude` 自行调整，当前脚本默认删除所有非 `brain` 成分。
...
`eeg_denoise_strong.py` performs offline strong denoising of one scalp-EEG EDF and writes a compressed NPZ.
Strong denoising means that ICLabel retains only `brain` components and removes every non-`brain` component without a confidence threshold.

## 2. Use Cases and Effects

- Suitable for standard monopolar scalp EEG, conventional 10-20/10-10 layouts, and 0.5–45 Hz analysis.
- It can reduce baseline drift, 50 Hz mains noise, high-frequency random noise, and components classified as non-brain.
...
- The current processing method, which only retains the `brain` signal, is too aggressive and may inadvertently delete real brain activity. It is necessary to check the waveform, spectrum, and ICA tags. See section 8 for details. You can select and retain tagged signals according to your research direction.
...
3. Apply a 0.5 Hz high-pass and a 50 Hz notch when 50 Hz is below Nyquist.
...
6. Classify with ICLabel, retaining only `brain` and removing every non-`brain` component.
...
| ICLabel default | Retain only `brain` | Strong denoising without confidence threshold; higher false-removal risk |
...
Denoising strength can be adjusted through ICA `exclude`; the current script removes every non-`brain` component.
