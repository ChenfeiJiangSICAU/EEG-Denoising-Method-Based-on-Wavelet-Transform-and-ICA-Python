...
def _apply_initial_filters(
    ...
    """Apply 0.5-Hz high-pass and the feasible 50-Hz notch in place."""
    ...
    nyquist = float(filtered.info["sfreq"]) / 2.0
    if config.notch_hz < nyquist:
        filtered.notch_filter(
            freqs=np.asarray([config.notch_hz]),
            picks=None,
            method="fir",
            phase="zero-double",
            fir_design="firwin",
        ...
        warnings.warn(
            f"Nyquist 频率 {nyquist:g} Hz 不高于 {config.notch_hz:g} Hz；"
            "数据中不存在可由数字滤波器处理的完整 50 Hz 频点，已跳过陷波。",
            RuntimeWarning,
            stacklevel=2,
        )
    return filtered
...
def _run_ica_iclabel(
    ...
    """Fit extended Infomax, keep only ICLabel components labeled brain."""
    ...
    excluded = [
        index for index, label in enumerate(labels) if label != "brain"
    ]
    kept = [
        index for index, label in enumerate(labels) if label == "brain"
    ]
    if not kept:
        raise RuntimeError(
            "ICLabel 没有识别出任何 brain 成分。为避免输出近乎空白的"
            "脑电，程序已安全中止；请检查通道名、蒙太奇、参考方式和"
            "原始数据质量。"
        )
    ...
    LOGGER.info("保留 brain 成分：%s", kept)
    LOGGER.info("剔除非 brain 成分：%s", excluded)
...
def save_npz(
    ...
    processing_parameters.update(
        {
            "ica_decim": ica_decim,
            "strict_iclabel_policy": "keep_only_brain",
...
def process_edf(
    ...
    *,
    ...
    with _stage(3, "0.5 Hz 高通和 50 Hz 陷波"):
        filtered_raw = _apply_initial_filters(prepared_raw, config)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "读取一个 EDF 患者脑电，依次执行 0.5 Hz 高通、50 Hz 陷波、"
